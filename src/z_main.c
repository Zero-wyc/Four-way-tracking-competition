...
const char *pre_cmd_set_grn[PSX_BUTTON_NUM] = {//绿灯模式下按键的配置			
	"<PS2_GRN01:$SMODE10!^$SMODE10!>",//L2						  
	"<PS2_GRN02:$SMODE10!^$SMODE10!>", //R2						  
	"<PS2_GRN03:$SMODE8!^$SMODE8!>",	 //L1						  
	"<PS2_GRN04:$SMODE9!^$SMODE9!>",	 //R1			
	"<PS2_GRN05:$SMODE4!^$SMODE4!>",	 //RU						  
	"<PS2_GRN06:$SMODE5!^$SMODE5!>",	 //RR						  
	"<PS2_GRN07:$SMODE7!^$SMODE7!>",	 //RD						  
	"<PS2_GRN08:$SMODE6!^$SMODE6!>",	 //RL				
	"<PS2_GRN09:$DJR!>",		 //SE    				  
	"<PS2_GRN10:>",					 //AL						   
	"<PS2_GRN11:>",					 //AR						  
	"<PS2_GRN12:$SMART_STOP!>",		 //ST		
	"<PS2_GRN13:$SMODE0!^$SMODE0!>",	 //LU						  
	"<PS2_GRN14:$SMODE3!^$SMODE3!>",	 //LR								  
	"<PS2_GRN15:$SMODE1!^$SMODE1!>",	 //LD						  
	"<PS2_GRN16:$SMODE2!^$SMODE2!>",	 //LL								
};
...
void loop_ps2_car(void) {
	...
	// 左摇杆Y轴(psx_buf[8])控制两个电机同向运动
	if(abs_int(127 - psx_buf[8]) < num )psx_buf[8] = 127;
	// 右摇杆X轴(psx_buf[5])控制两个电机反向运动实现转向
	if(abs_int(127 - psx_buf[5]) < num )psx_buf[5] = 127;
	
	// 检测摇杆是否推到底，启动电机控制
	if(abs_int(127 - psx_buf[8]) > 100) {
		car_flag = 1;
		systick_ms_bak = millis();
	}
	
	// 左摇杆Y轴控制前进后退
	speed = (127 - psx_buf[8]) * 8;
	// 右摇杆X轴控制转向
	turn = (127 - psx_buf[5]) * 8;
	
	car_left = speed - turn;
	car_right = speed + turn;
...
void car_set(int car_left,int car_right) {
	...
	if(car_left != car_left_bak || car_right != car_right_bak){
			//总线马达设置	
		sprintf((char *)cmd_return, "{#006P%04dB0000!#007P%04dB0000!}", (int)(1500+car_left), (int)(1500-car_right));
		zx_uart_send_str(cmd_return);
...
void car_move(int x, int w)
{
	...
	car_left1 = x + w;	// 前左轮速度
	car_right1 = x - w; // 前右轮速度

	sprintf((char *)cmd_return, "{#006P%04dB0000!#007P%04dB0000!}",
			(int)(1500 + car_left1), (int)(1500 - car_right1));
	zx_uart_send_str(cmd_return);
	return;
}
...
void parse_cmd(u8 *cmd) {
	...
	if(pos = str_contain_str(cmd, (u8 *)"$DRS!"), pos) {
		...
		mdelay(10);
		zx_uart_send_str((u8 *)"{#006P1500T0000!#007P1500T0000!}");
		mdelay(10);
		uart1_send_str((u8 *)"@OK!");
		mdelay(10);
		beep_on_times(1,100);
